...
public class MyPageController {
	...
	public String main(Model model, HttpSession session) throws Exception{
		...
		model.addAttribute("followCnt", userService.selectFollowCnt(user_id));
...
//		}
	...
	@RequestMapping(value = "/petfollow/list.do")
	public String selectFollowList(HttpSession session, Model model) {
		
		LOGGER.info("follow List");
		
		String user_id = (String)session.getAttribute("user_id");
		List<String> arFollow_id = userService.selectFollowList(user_id);
		
		ArrayList<UserVO> list = new ArrayList<UserVO>();
		
		for(String follow_id : arFollow_id) {
			UserVO user = userService.getUser(follow_id);
			user.setPet_photothumb(userService.getPet(follow_id).getPet_photothumb());
			user.setPet_name(userService.getPet(follow_id).getPet_name());
			list.add(user);
		}
		
		model.addAttribute("list", list);
		
		return "/mypage/petfollow/list";
	}
}
