...
DELETE FROM TBL_USER WHERE USER_ID = 'test';
DELETE FROM TBL_PET WHERE USER_ID = 'test';
...
SELECT
			...
			tb."FILE",
			tu.USER_NAME 
			FROM (SELECT * FROM TBL_BOARD WHERE NUM > 0 ORDER BY NUM DESC, "DATE" DESC) tb
		JOIN TBL_USER tu ON tb.USER_ID = tu.USER_ID 
